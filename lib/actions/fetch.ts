...
export const fetchMultipleGiveState = (payload) => (dispatch, getState) =>
  runFetchMultipleGiveState(payload, dispatch, getState())
 ...
 * @returns Promise
 */
function runFetchMultipleGiveState(
  {
    type = GFT,
    id = getID(), // One ID for all fetch IDs in a fetch multiple
    fetches,
  ...
  return Promise.all(
    ...
    .then((responses) => {
      if (fetchIsActive(signature)) {
        dispatch(decrementFetches(signature))
        if (next) dispatch(wrapNextGiveState(next)(responses,state))
        // Just return the values for Promise based execution
        return responses.map((r) => r.value || r)
      }
    })
...
function wrapNextGiveState(next: NextFn) {
  return function (response: FetchResponse | FetchResponse[],state: State) {
    if (next.length > 1) {
      return next(null, response, state)
    } else {
      return next(response)
    }
  }
